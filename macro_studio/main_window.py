...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def _build_sidebar(self) -> QtWidgets.QWidget:
        ...
        for key, label in NAV_ITEMS:
            ...
            if key == "export":
                layout.addStretch(1)
        self.btn_mini_hud = QtWidgets.QPushButton("🪟  플로팅 미니 HUD")
        self.btn_mini_hud.setObjectName("Nav")
        self.btn_mini_hud.setToolTip("화면 구석에 항상 떠있는 미니 제어 HUD를 띄웁니다.")
        self.btn_mini_hud.clicked.connect(self._toggle_mini_hud)
        layout.addWidget(self.btn_mini_hud)
    ...
    def _apply_sidebar_state(self, save: bool = True) -> None:
        ...
        if hasattr(self, "btn_mini_hud"):
            self.btn_mini_hud.setText("🪟" if collapsed else "🪟  플로팅 미니 HUD")
        self._refresh_nav_labels()
        if save:
            self.settings.setValue("sidebar_collapsed", collapsed)

    def _toggle_mini_hud(self) -> None:
        if not hasattr(self, "_mini_hud") or not self._mini_hud:
            return
        if self._mini_hud.isVisible():
            self._mini_hud.hide()
        else:
            self._mini_hud.show()
    ...
    def changeEvent(self, event: QtCore.QEvent) -> None:
        if event.type() == QtCore.QEvent.WindowStateChange:
            if self.isMinimized() and self._running_macro_processes:
    ...
    def _register_studio_shortcuts(self, saved: dict | None = None) -> None:
        ...
        callbacks = {
            ...
            "action_help": self._open_help_guide,
        }
        for action_id, _label, default in STUDIO_SHORTCUT_SPECS:
    ...
    def _update_macro_run_state(self) -> None:
        ...
        if hasattr(self, "_mini_hud") and self._mini_hud and not self._running_macro_processes:
            self._mini_hud.update_status("", 0, 0, "", False, False)
    ...
    def _poll_running_macros(self) -> None:
        ...
        for pid, (name, process, result_path, progress_path, click_path) in tuple(
            ...
            if int(return_code) != 0 and status not in {"FAILED", "PARTIAL"}:
                ...
                status, code, detail = "SUCCESS", "COMPLETED", "매크로 정상 완료"
            level = {"SUCCESS": "OK", "PARTIAL": "WARN", "FAILED": "ERROR"}.get(status, "INFO")

            # Translate technical error codes to clear cause & actionable advice
            friendly_reason = detail
            action_guide = ""
            if "IMAGE_NOT_FOUND" in code or "IMAGE_NOT_FOUND" in detail:
                friendly_reason = "화면에서 대상 이미지를 찾지 못함"
                action_guide = "💡 [원인] 화면에 대상이 없거나/창 가려짐/일치율 미달 | 🔧 [해결] 대상 창 확인, 상세설정에서 일치 신뢰도를 낮추거나 범위를 전체 화면으로 확장"
            elif "AHK_IMAGE_ERROR" in code or "IMAGE_FILE_MISSING" in code:
                friendly_reason = "이미지 파일을 불러올 수 없음"
                action_guide = "💡 [원인] 이미지 파일이 손상되었거나 삭제됨 | 🔧 [해결] 노드 상세설정에서 이미지를 다시 캡처해주세요"
            elif "TARGET_WINDOW_NOT_FOUND" in code:
                friendly_reason = "대상 프로그램 창을 찾지 못함"
                action_guide = "💡 [원인] 대상 앱이 켜져 있지 않음 | 🔧 [해결] 대상 프로그램을 먼저 실행한 후 매크로를 시작하세요"
            elif "TARGET_WINDOW_INACTIVE" in code:
                friendly_reason = "대상 창 활성화 실패"
                action_guide = "💡 [원인] 창이 최소화되었거나 관리자 권한 부족 | 🔧 [해결] 창 최소화 해제 및 스튜디오를 관리자 권한으로 실행"
            elif "EXIT_" in code:
                friendly_reason = f"비정상 종료 (코드 {return_code})"
                action_guide = "💡 [원인] 프로세스 예기치 않은 종료 | 🔧 [해결] 백그라운드 프로그램 충돌 여부 및 관리자 권한 확인"

            status_kor = "성공" if status == "SUCCESS" else ("일부 완료" if status == "PARTIAL" else "실패")
            log_line = f"[{status_kor}] {name} | 내용: {friendly_reason}"
            if action_guide:
                log_line += f"\n  ➔ {action_guide}"
            self._append_run_log(log_line, level)
            ...
            if status == "FAILED":
                if not any(key[0] == pid for key in self._failure_capture_steps):
                    ...
                    if failure_capture is not None:
                        ...
                        self._append_run_log(f"실패 화면 캡처 저장 | {failure_capture}", "ERROR")
                short_guide = action_guide.split("🔧 [해결]")[-1].strip() if action_guide else "로그를 확인하세요."
                self.show_status(f"'{name}' 실패: {friendly_reason} (조치: {short_guide})")
            elif status == "PARTIAL":
                self.show_status(f"'{name}' 일부 완료: {friendly_reason}")
            else:
                self.show_status(f"'{name}' 실행을 성공적으로 마쳤습니다.")
    ...
    def _set_running_node(self, display_name: str, step: int) -> None:
        ...
        if hasattr(self, "_mini_hud") and self._mini_hud:
            total_steps = 0
            step_label = ""
            if builder.current_macro:
                steps = builder.current_macro.get("steps") or []
                total_steps = len(steps)
                if 0 < step <= total_steps:
                    curr = steps[step - 1]
                    step_label = curr.get("label") or curr.get("action") or f"{step}번 노드"
            is_running = bool(self._running_macro_processes)
            is_paused = False
            for cpath in self._run_control_paths.values():
                if cpath and cpath.is_file():
                    try:
                        if cpath.read_text(encoding="utf-8-sig").strip().upper() == "PAUSE":
                            is_paused = True
                            break
                    except OSError:
                        pass
            self._mini_hud.update_status(macro_name, step, total_steps, step_label, is_running, is_paused)
    ...
    def _update_recent_click_preview(self, pid: int, path: Path | None) -> bool:
        ...
        if pixmap is not None:
            self._last_click_pixmap = pixmap
            if hasattr(self, "_mini_hud") and self._mini_hud:
                self._mini_hud.update_click_preview(pixmap)
        builder = self.pages.get("builder")
