...
def debug_variable_names(steps: Iterable[Dict[str, Any]]) -> List[str]:
    ...
    for step in steps:
        ...
        candidates.append(step.get("result_var"))
        for mapping_key in ("inputs", "outputs", "_subflow_inputs", "_subflow_outputs"):
            mapping = step.get(mapping_key)
            if isinstance(mapping, dict):
                candidates.extend(mapping.keys())
                candidates.extend(str(value).lstrip("$") for value in mapping.values())
...
def build_macro_header(macro: Dict[str, Any]) -> List[str]:
    ...
    failure_limit = max(0, int(macro.get("failure_streak_limit") or meta.get("failure_streak_limit") or 0))
    stuck_guard_limit = max(0, int(macro.get("stuck_guard_limit") or meta.get("stuck_guard_limit") or 0))
    return [
        ...
        "EnvGet, MacroRunCheckpointFile, MACRORELAY_CHECKPOINT_FILE",
        "EnvGet, MacroRunResumeStep, MACRORELAY_RESUME_STEP",
        "EnvGet, MacroRunDryRun, MACRORELAY_DRY_RUN",
        'MacroDryRun := (MacroRunDryRun = "1" or MacroRunDryRun = "true" or MacroRunDryRun = "TRUE")',
        "EnvGet, MacroRunTurboMode, MACRORELAY_TURBO_MODE",
        'MacroTurboMode := (MacroRunTurboMode = "1" or MacroRunTurboMode = "true" or MacroRunTurboMode = "TRUE")',
        "if (MacroTurboMode) {",
        "    SetKeyDelay, -1, -1",
        "    SetMouseDelay, -1",
        "    SetWinDelay, -1",
        ...
        "MacroConsecutiveFailures := 0",
        f"MacroFailureLimit := {failure_limit}",
        "SetRunProgress(step) {",
        "    global MacroRunProgressFile",
        "    if (MacroRunProgressFile = \"\")",
        "        return",
        "    try {",
        '        pfile := FileOpen(MacroRunProgressFile, "w", "UTF-8")',
        '        if (pfile) {',
        '            pfile.Write(step)',
        '            pfile.Close()',
        '        }',
        ...
        "}",
        "MarkStepSuccess(step, nextStep := 0) {",
        "    global MacroRunCheckpointFile, MacroConsecutiveFailures, MacroRunStatus",
        "    MacroConsecutiveFailures := 0",
        '    if (MacroRunStatus = "FAILED" or MacroRunStatus = "PARTIAL")',
        '        SetRunResult("RUNNING", "RECOVERED", "재시도 또는 대체 흐름에서 복구됨")',
        '    if (MacroRunCheckpointFile = "" or nextStep <= 0)',
        "        return",
        "    FileDelete, %MacroRunCheckpointFile%",
        "    FileAppend, %nextStep%, %MacroRunCheckpointFile%, UTF-8",
        "}",
        "MarkStepFailure(step) {",
        "    global MacroConsecutiveFailures, MacroFailureLimit",
        "    MacroConsecutiveFailures += 1",
        '    TraceStep(step, "연속 실패 감시", "DETAIL", "streak=" . MacroConsecutiveFailures . "; limit=" . MacroFailureLimit)',
        "    if (MacroFailureLimit > 0 and MacroConsecutiveFailures >= MacroFailureLimit)",
        "    {",
        '        SetRunResult("FAILED", "FAILURE_STREAK_LIMIT", "연속 실패 제한에 도달해 자동 정지했습니다.")',
        "        ExitApp, 7",
        "    }",
        "}",
        "ClearRunCheckpoint() {",
        "    global MacroRunCheckpointFile",
        '    if (MacroRunCheckpointFile != "")',
        "        FileDelete, %MacroRunCheckpointFile%",
        "}",
        "TraceStep(step, label, status, detail := \"\") {",
        "    global MacroRunTraceFile, MacroTurboMode",
        "    if (MacroRunTraceFile = \"\" or MacroTurboMode)",
        "        return",
        ...
        *debug_variable_helpers(macro),
        ...
        f"StuckGuard_Limit := {stuck_guard_limit}",
        "StuckGuard_Check(step, label) {",
        "    global StuckGuard_LastStep, StuckGuard_Consecutive, StuckGuard_Limit, PythonExe",
        "    if (StuckGuard_Limit <= 0)",
        "        return",
        "    if (step = StuckGuard_LastStep) {",
        "        StuckGuard_Consecutive += 1",
        "        if (StuckGuard_Consecutive >= StuckGuard_Limit) {",
        "            Log(\"STUCK GUARD TRIGGERED: step \" . step . \" repeated \" . StuckGuard_Consecutive . \" times\")",
        "            SoundBeep, 750, 350",
        "            SoundBeep, 850, 350",
        "            __sg_notify := A_ScriptDir . \"\\remote_notify.py\"",
        "            if !FileExist(__sg_notify)",
        "                __sg_notify := A_ScriptDir . \"\\..\\remote_notify.py\"",
        "            if FileExist(__sg_notify) {",
        "                __sg_msg := \"[\" . step . \"번 노드: \" . label . \"] \" . StuckGuard_Limit . \"회 연속 반복(스턱)이 감지되어 매크로가 안전하게 정지되었습니다.\"",
        "                __sg_cmd := \"\"\"\" . PythonExe . \"\"\" \"\"\" . __sg_notify . \"\"\" --title \"\"🚨 [무인 자동화] 스턱 감지 알림\"\" --level \"\"error\"\" --message \"\"\" . __sg_msg . \"\"\"\"",
        "                Run, %__sg_cmd%, , Hide",
        "            }",
        "            SetRunResult(\"FAILED\", \"STUCK_GUARD\", \"같은 노드의 연속 반복 제한에 도달했습니다.\")",
        "            ExitApp, 8",
        "            StuckGuard_Consecutive := 0",
        "        }",
        "    } else {",
        "        StuckGuard_LastStep := step",
        "        StuckGuard_Consecutive := 1",
        "    }",
        "}",
...
def render_inactive_click(step: Dict[str, Any]) -> List[str]:
    ...
    if action_type == "drag" and isinstance(drag_to, list) and len(drag_to) >= 2:
        ...
        effective_button = "Right" if (str(button).lower() == "right" or action_type == "right_click") else str(button)
        if method == "postmessage":
            ...
            if str(button).lower() in ("wheelup", "wheeldown"):
                ...
                down_msg = "0x204" if effective_button.lower() == "right" else "0x201"
                up_msg = "0x205" if effective_button.lower() == "right" else "0x202"
                down_wparam = "2" if effective_button.lower() == "right" else "1"

                if action_type == "down":
                    lines.append("PostMessage, 0x200, 0, %lParam%, , " + click_token)
                    lines.append(f"PostMessage, {down_msg}, {down_wparam}, %lParam%, , {click_token}")
                    if direct_post:
                        lines.append('Log("inactive click direct post: down sent")')
                elif action_type == "up":
                    lines.append(f"PostMessage, {up_msg}, 0, %lParam%, , {click_token}")
                    if direct_post:
                        lines.append('Log("inactive click direct post: up sent")')
                else:
                    lines.append("PostMessage, 0x200, 0, %lParam%, , " + click_token)
                    lines.append(f"PostMessage, {down_msg}, {down_wparam}, %lParam%, , {click_token}")
                    if press_duration > 0:
            ...
            if action_type == "down":
                lines.append("    lParam := (ClickY << 16) | (ClickX & 0xFFFF)")
                lines.append('    __down := (__click_button = "Right") ? 0x204 : 0x201')
                ...
                lines.append('    Log("inactive click auto: down sent")')
                lines.append("    __click_ok := 1")
            elif action_type == "up":
                lines.append("    lParam := (ClickY << 16) | (ClickX & 0xFFFF)")
                lines.append('    __up := (__click_button = "Right") ? 0x205 : 0x202')
                lines.append("    PostMessage, %__up%, 0, %lParam%, , ahk_id %ClickHwnd%")
                lines.append('    Log("inactive click auto: up sent")')
                lines.append("    __click_ok := 1")
            else:
...
def render_edge_conditions(
    ...
    for rule in rules:
        ...
        if kind == "success":
            lines.append(indent + f"    MarkStepSuccess({step_index}, {target})")
...
def render_text_condition(step: Dict[str, Any], step_index: int) -> List[str]:
    ...
    if on_no_match_delay < 0:
        on_no_match_delay = 0
    retry_count = max(0, int(step.get("node_retry_count") or 0))
    retry_delay = max(10, int(step.get("node_retry_delay") or 250))
    ...
    lines.append("{")
    lines.append(f"    __node_retry_{step_index} := 0")
    lines.extend("    " + line for line in render_subflow_success(step))
    lines.append(f"    MarkStepSuccess({step_index}, {on_match or step_index + 1})")
    ...
    lines.append("{")
    if retry_count:
        lines.append(f"    if (__node_retry_{step_index} < {retry_count})")
        lines.append("    {")
        lines.append(f"        __node_retry_{step_index} += 1")
        lines.append(
            f'        TraceStep({step_index}, "텍스트 조건", "RETRY", "attempt=" . __node_retry_{step_index} . "/{retry_count}")'
        )
        lines.append('        SetRunResult("RUNNING", "RETRYING", "노드 재시도 중")')
        lines.append(f"        Sleep, {retry_delay}")
        lines.append(f"        Goto, Step{step_index}")
        lines.append("    }")
    lines.append(f"    __node_retry_{step_index} := 0")
    lines.extend("    " + line for line in render_subflow_failure(step))
    lines.append(f"    MarkStepFailure({step_index})")
...
def render_subflow_assignments(mapping: Any, output: bool = False) -> List[str]:
    """Render safe variable copies/literals used at a subflow boundary."""
    if not isinstance(mapping, dict):
        return []
    lines: List[str] = []
    for raw_target, raw_value in mapping.items():
        target = normalize_variable_name(raw_target)
        if not target:
            continue
        value = str(raw_value or "").strip()
        source = normalize_variable_name(value.lstrip("$"))
        if source and (output or value.startswith("$")):
            lines.append(f"{target} := {source}")
            continue
        lowered = value.casefold()
        if lowered in {"true", "false"}:
            lines.append(f"{target} := {1 if lowered == 'true' else 0}")
            continue
        try:
            float(value)
        except ValueError:
            lines.append(f'{target} := "{ahk_quote(value)}"')
        else:
            lines.append(f"{target} := {value}")
    return lines


def render_subflow_success(step: Dict[str, Any]) -> List[str]:
    lines = render_subflow_assignments(step.get("_subflow_outputs"), output=True)
...
def render_table_paste(step: Dict[str, Any]) -> List[str]:
    ...
    if mode == "inactive":
        lines.append(f'TargetHwnd := EnsureTargetWindow(TargetHwnd, "{ahk_quote(str(window))}", "{ahk_quote(str(window_exe))}")')
        lines.append("if (TargetHwnd)")
        lines.append("{")
...
def render_image_search(
    ...
    def emit_opencv_search(region_vars, indent=""):
        ...
        capture_cache_ms = max(0, min(250, int(step.get("capture_cache_ms") or 45)))
        capture_context = ahk_quote(f"{region_mode}|{region_window}|{region_window_exe}")
        lines.append(f'{indent}VisionContextJson := VisionEngine_JsonEscape("{capture_context}")')
        ...
        raw_asset_confidences = step.get("asset_confidences") if isinstance(step.get("asset_confidences"), dict) else {}
        asset_thresholds: list[float] = []
        if len(multi_image_vars) > 1 and raw_asset_confidences:
            for cand_alias in aliases:
                c_val = raw_asset_confidences.get(cand_alias)
                if c_val is not None:
                    try:
                        asset_thresholds.append(round(max(0.01, min(0.99, float(c_val) / 100)), 2))
                    except Exception:
    ...
    if engine == "opencv":
        ...
        if len(regions_list) == 1:
            ...
            if timeout:
                ...
                break_cond = "ErrorLevel != 0" if wait_cond == "vanish" else "ErrorLevel = 0"
                lines.append(f"    if ({break_cond})")
                lines.append("        break")
    ...
    wait_cond = str(step.get("wait_condition") or "appear").lower()
    if wait_cond == "vanish":
        lines.append("; 이미지 사라짐 판정: ErrorLevel != 0 이면 사라짐 성공")
        lines.append("if (ErrorLevel != 0) {")
        lines.append(f"    {found_var} := 1")
        lines.append("    ErrorLevel := 0")
        lines.append("} else {")
    ...
    lines.append("if ErrorLevel")
    lines.append("{")
    if bool(step.get("fail_click_enabled")):
        fail_offset = step.get("fail_click_offset") or [0, 0]
        fail_x = int(fail_offset[0] or 0) if isinstance(fail_offset, (list, tuple)) and len(fail_offset) >= 2 else 0
        fail_y = int(fail_offset[1] or 0) if isinstance(fail_offset, (list, tuple)) and len(fail_offset) >= 2 else 0
        fail_mode = str(step.get("fail_click_mode") or "inactive").lower()
        fail_count = max(1, int(step.get("fail_click_count") or 1))
        lines.append(f'    ; ── 검색 실패 시 대체 클릭 ──')
        lines.append(f'    Log("🎯 [대체 클릭 시작] 이미지 미탐지로 인해 지정 좌표(X={fail_x}, Y={fail_y})로 {fail_count}회 대체 클릭을 실행합니다. (방식: {fail_mode})")')
        lines.append(f"    if (!MacroDryRun)")
        lines.append(f"    {{")
        if fail_mode == "active":
            lines.append(f"        CoordMode, Mouse, Screen")
            lines.append(f"        Click, {fail_x}, {fail_y}, {fail_count}")
            lines.append(f"        CoordMode, Mouse, %MacroMouseCoordMode%")
            lines.append(f'        SetLastClick({fail_x}, {fail_y}, "foreground")')
        else:
            lines.append(f"        VarSetCapacity(__f_pt, 8, 0)")
            lines.append(f"        NumPut({fail_x}, __f_pt, 0, \"Int\")")
            lines.append(f"        NumPut({fail_y}, __f_pt, 4, \"Int\")")
            lines.append(f'        __fail_hwnd := DllCall("WindowFromPoint", "Int64", ({fail_y} << 32) | ({fail_x} & 0xFFFFFFFF), "Ptr")')
            lines.append(f"        if (__fail_hwnd)")
            lines.append(f"        {{")
            lines.append(f'            DllCall("ScreenToClient", "Ptr", __fail_hwnd, "Ptr", &__f_pt)')
            lines.append(f'            __fail_cx := NumGet(__f_pt, 0, "Int")')
            lines.append(f'            __fail_cy := NumGet(__f_pt, 4, "Int")')
            lines.append(f"            __fail_lparam := (__fail_cy << 16) | (__fail_cx & 0xFFFF)")
            lines.append(f"            Loop, {fail_count}")
            lines.append(f"            {{")
            lines.append(f"                PostMessage, 0x201, 1, %__fail_lparam%,, ahk_id %__fail_hwnd%")
            lines.append(f"                Sleep, 35")
            lines.append(f"                PostMessage, 0x202, 0, %__fail_lparam%,, ahk_id %__fail_hwnd%")
            lines.append(f"                ControlClick, x%__fail_cx% y%__fail_cy%, ahk_id %__fail_hwnd%,, Left, 1, NA")
            lines.append(f"                if (A_Index < {fail_count})")
            ...
            lines.append(f"            CoordMode, Mouse, %MacroMouseCoordMode%")
            lines.append(f"        }}")
            lines.append(f'        SetLastClick({fail_x}, {fail_y}, "inactive")')
        lines.append(f"    }}")
        lines.append(f'    Log("🎯 [대체 클릭 완료] 좌표(X={fail_x}, Y={fail_y}) 클릭 전송이 정상 완료되었습니다.")')
    ...
    if die_on_missing:
        ...
        lines.append('    Log("click skipped: image not found")')
        lines.append(f'    Log("💡 [조치 안내] 대상을 꼭 찾아야 한다면 상세설정에서 신뢰도를 낮추거나 범위를 넓혀보세요.")')
    ...
    if click_info and all_action != "count_only":
        ...
        def emit_click(info, offset_override=None):
            ...
            lines.append("    if (MacroDryRun)")
            lines.append("    {")
            lines.extend(f"        {line}" for line in render_click_preview_from_hit(info_use, "image"))
            lines.append("    }")
            lines.append("    else")
            lines.append("    {")
            if mode == "inactive":
...
def render_ocr_engine(step: Dict[str, Any]) -> List[str]:
    ...
    if ocr_action_type in ("find_click", "find_click_offset"):
        lines.extend([
            ...
            '        if (MacroDryRun)',
            '            SetLastClick(__click_x, __click_y, "dry-run-ocr")',
            '        else',
            '        {',
...
def render_click_preview_from_hit(click_info: Dict[str, Any], kind: str = "image") -> List[str]:
    click_type = click_info.get("type", "relative")
    if click_type == "absolute":
        x_expr = str(click_info.get("x", 0))
        y_expr = str(click_info.get("y", 0))
    else:
        offsets = click_info.get("offset", [0, 0])
        offset_x, offset_y = (offsets + [0, 0])[:2]
        scale_x = str(click_info.get("_offset_scale_x") or "")
    ...
    return [
        f"ClickX := {x_expr}",
        f"ClickY := {y_expr}",
        'Log("dry-run predicted click: " . ClickX . "," . ClickY)',
        f'SetLastClick(ClickX, ClickY, "dry-run-{kind}")',
    ]
...
def render_ocr_tracking(step: Dict[str, Any], assets: Dict[str, Dict[str, Any]]) -> List[str]:
    """Render OCR tracking: find tracking image and/or color, then OCR at offset."""
    tracking_mode = str(step.get("tracking_mode") or "image").lower()
    tracking_asset = str(step.get("tracking_asset") or "")
    tracking_confidence = int(step.get("tracking_confidence") or 80)
    tracking_color = str(step.get("tracking_color") or "#FF0000").strip()
    color_hex = "0x" + (tracking_color[1:] if tracking_color.startswith("#") else tracking_color).upper()
    tracking_tolerance = int(step.get("tracking_tolerance") or 15)
    tracking_color_offset_x = int(step.get("tracking_color_offset_x") or 0)
    tracking_color_offset_y = int(step.get("tracking_color_offset_y") or 0)

    ocr_offset_x = int(step.get("ocr_offset_x") or 0)
    ocr_offset_y = int(step.get("ocr_offset_y") or 0)
    ocr_width = int(step.get("ocr_width") or 200)
    ocr_height = int(step.get("ocr_height") or 50)
    lang = str(step.get("lang") or "eng+kor")
    engine_pref = str(step.get("engine_preference") or "auto")
    ocr_action = str(step.get("ocr_action") or "extract")
    find_text = str(step.get("find_text") or "")
    match_mode = str(step.get("match_mode") or "contains")
    store_var = str(step.get("store_var") or "")
    timeout = int(step.get("timeout") or 5000)
    poll_delay = int(step.get("poll_delay") or 100)
    asset_info = assets.get(tracking_asset, {})
    image_path = str(asset_info.get("path") or asset_info.get("file") or tracking_asset)

    lines: List[str] = []
    lines.append(f"; ── OCR 추적 ({tracking_mode}) ──")
    lines.append(f"OcrTrack_Timeout := {timeout}")
    lines.append(f"OcrTrack_PollDelay := {poll_delay}")
    lines.append("OcrTrack_Start := A_TickCount")
    lines.append("OcrTrack_Found := false")
    lines.append("Loop {")

    if tracking_mode == "color":
        lines.append(f"    PixelSearch, OcrTrack_FoundX, OcrTrack_FoundY, 0, 0, A_ScreenWidth, A_ScreenHeight, {color_hex}, {tracking_tolerance}, Fast RGB")
        lines.append("    if (ErrorLevel = 0) {")
        lines.append("        OcrTrack_Found := true")
        lines.append("        break")
        lines.append("    }")
    elif tracking_mode == "both_and":
        lines.append(f'    ImageSearch, OcrTrack_FoundX, OcrTrack_FoundY, 0, 0, A_ScreenWidth, A_ScreenHeight, *{tracking_confidence} "{image_path}"')
        lines.append("    if (ErrorLevel = 0) {")
        lines.append(f"        __cx := OcrTrack_FoundX + {tracking_color_offset_x}")
        lines.append(f"        __cy := OcrTrack_FoundY + {tracking_color_offset_y}")
        lines.append("        PixelGetColor, __clr, %__cx%, %__cy%, RGB")
        lines.append("        __r_cur := (__clr >> 16) & 0xFF, __g_cur := (__clr >> 8) & 0xFF, __b_cur := __clr & 0xFF")
        lines.append(f"        __r_tgt := ({color_hex} >> 16) & 0xFF, __g_tgt := ({color_hex} >> 8) & 0xFF, __b_tgt := {color_hex} & 0xFF")
        lines.append("        __diff := Abs(__r_cur - __r_tgt) + Abs(__g_cur - __g_tgt) + Abs(__b_cur - __b_tgt)")
        lines.append(f"        if (__diff <= ({tracking_tolerance} * 3)) {{")
        lines.append("            OcrTrack_Found := true")
        lines.append("            break")
        lines.append("        }")
        lines.append("    }")
    elif tracking_mode == "either_or":
        lines.append(f'    ImageSearch, OcrTrack_FoundX, OcrTrack_FoundY, 0, 0, A_ScreenWidth, A_ScreenHeight, *{tracking_confidence} "{image_path}"')
        lines.append("    if (ErrorLevel = 0) {")
        lines.append("        OcrTrack_Found := true")
        lines.append("        break")
    ...
    lines.append("    if (A_TickCount - OcrTrack_Start >= OcrTrack_Timeout)")
    lines.append("        break")
    lines.append(f"    Sleep, %OcrTrack_PollDelay%")
    lines.append("}")
    lines.append("; 2단계: 추적 위치 기준 OCR 영역 계산")
    lines.append("if (OcrTrack_Found) {")
    lines.append(f"    OcrTrack_OcrLeft := OcrTrack_FoundX + {ocr_offset_x}")
    lines.append(f"    OcrTrack_OcrTop := OcrTrack_FoundY + {ocr_offset_y}")
    lines.append(f"    OcrTrack_OcrRight := OcrTrack_OcrLeft + {ocr_width}")
    lines.append(f"    OcrTrack_OcrBottom := OcrTrack_OcrTop + {ocr_height}")
    lines.append("    ; 3단계: OCR 엔진으로 텍스트 인식")
    ocr_json_parts = [
        '""cmd"": ""ocr""',
        '""left"": " . OcrTrack_OcrLeft . "',
        '""top"": " . OcrTrack_OcrTop . "',
        '""right"": " . OcrTrack_OcrRight . "',
        '""bottom"": " . OcrTrack_OcrBottom . "',
        f'""lang"": ""{lang}""',
        f'""engine_preference"": ""{engine_pref}""',
        f'""ocr_action"": ""{ocr_action}""',
    ]
    if find_text:
        ocr_json_parts.append(f'""find_text"": ""{find_text}""')
        ocr_json_parts.append(f'""match_mode"": ""{match_mode}""')
    json_expr = '"{{" . ' + ' . ", " . '.join(ocr_json_parts) + ' . "}}"'
    lines.append(f"    OcrTrack_Json := {json_expr}")
    lines.append("    OcrEngine_Send(OcrTrack_Json, OcrTrack_Response, 9234)")
    lines.append("    OcrEngine_Parse(OcrTrack_Response)")
    if store_var:
        lines.append(f"    {store_var} := OCR_LastText")
    lines.append("}")
    return lines
...
def render_multi_pixel_check(step: Dict[str, Any]) -> List[str]:
    ...
    try:
        ...
        elif isinstance(pixels_raw, list):
            pixels = pixels_raw
        else:
            pixels = []
    except Exception:
...
def render_wait_color(step: Dict[str, Any]) -> List[str]:
    """Render AHK code waiting for a pixel color to become or leave a target color."""
    x = int(step.get("x") or 0)
    y = int(step.get("y") or 0)
    target_color = str(step.get("target_color") or "#FF0000").strip()
    color_hex = "0x" + (target_color[1:] if target_color.startswith("#") else target_color).upper()
    condition = str(step.get("condition") or "become").lower()
    tolerance = int(step.get("tolerance") or 10)
    timeout = int(step.get("timeout") or 5000)
    poll_delay = int(step.get("poll_delay") or 50)
    sleep_after = int(step.get("sleep_after") or 0)
    ...
    has_region = False
    if isinstance(region, (list, tuple)) and len(region) >= 4:
        r_left, r_top, r_right, r_bottom = int(region[0]), int(region[1]), int(region[2]), int(region[3])
        if r_right > r_left and r_bottom > r_top:
            has_region = True

    lines: List[str] = [
        f"; ── 색상 변화 대기 ({condition}{' · 영역' if has_region else ''}) ──",
        f"WaitColor_Target := {color_hex}",
        f"WaitColor_Tolerance := {tolerance}",
        f"WaitColor_Timeout := {timeout}",
        f"WaitColor_PollDelay := {poll_delay}",
        "WaitColor_Start := A_TickCount",
        "WaitColor_Success := false",
        "Loop {",
    ]
    if has_region:
        lines.append(f"    PixelSearch, __px, __py, {r_left}, {r_top}, {r_right}, {r_bottom}, %WaitColor_Target%, %WaitColor_Tolerance%, Fast RGB")
        if condition == "leave":
            lines.extend([
                "    if (ErrorLevel != 0) {",
                "        WaitColor_Success := true",
                "        break",
                "    }",
            ])
        else:
        ...
        lines.extend([
            f"    PixelGetColor, __cur_clr, {x}, {y}, RGB",
            "    __r_cur := (__cur_clr >> 16) & 0xFF, __g_cur := (__cur_clr >> 8) & 0xFF, __b_cur := __cur_clr & 0xFF",
            f"    __r_tgt := ({color_hex} >> 16) & 0xFF, __g_tgt := ({color_hex} >> 8) & 0xFF, __b_tgt := {color_hex} & 0xFF",
            "    __diff := Abs(__r_cur - __r_tgt) + Abs(__g_cur - __g_tgt) + Abs(__b_cur - __b_tgt)",
        ])
        if condition == "leave":
            lines.extend([
                f"    if (__diff > ({tolerance} * 3)) {{",
                "        WaitColor_Success := true",
                "        break",
                "    }",
            ])
    ...
    lines.extend([
        "    if (A_TickCount - WaitColor_Start >= WaitColor_Timeout)",
        "        break",
        f"    Sleep, %WaitColor_PollDelay%",
        "}",
    ])
    if sleep_after > 0:
        lines.append(f"Sleep, {sleep_after}")
...
def render_color_ratio(step: Dict[str, Any]) -> List[str]:
    """Render AHK/Python code calculating the pixel count and ratio percentage of a color in a region."""
    target_color = str(step.get("target_color") or "#FF0000").strip()
    color_hex = target_color if target_color.startswith("#") else f"#{target_color}"
    tolerance = int(step.get("tolerance") or 15)
    region = step.get("search_region") or [0, 0, 0, 0]
    if isinstance(region, (list, tuple)) and len(region) >= 4:
        left, top, right, bottom = int(region[0]), int(region[1]), int(region[2]), int(region[3])
    else:
        left, top, right, bottom = 0, 0, 0, 0
    store_ratio = str(step.get("store_ratio_var") or "GaugePercent")
    store_count = str(step.get("store_count_var") or "PixelCount")
    op = str(step.get("condition_operator") or "none").lower()
    val = int(step.get("condition_value") or 30)
    sleep_after = int(step.get("sleep_after") or 0)

    lines: List[str] = [
        f"; ── 색상 비율 게이지 ({color_hex}) ──",
        f"{store_ratio} := 0",
        f"{store_count} := 0",
        f'__ratio_cmd := PythonExe . " -c ""from PIL import ImageGrab; import numpy as np; '
        f'img = ImageGrab.grab(bbox=({left},{top},{right},{bottom})); arr = np.array(img)[:,:,:3]; '
        f'r,g,b = {int(color_hex[1:3], 16)}, {int(color_hex[3:5], 16)}, {int(color_hex[5:7], 16)}; '
        f'diff = np.abs(arr[:,:,0]-r) + np.abs(arr[:,:,1]-g) + np.abs(arr[:,:,2]-b); '
        f'cnt = int(np.sum(diff <= ({tolerance} * 3))); total = max(1, arr.shape[0]*arr.shape[1]); '
        f'pct = round(cnt / total * 100, 1); print(f\'{{cnt}},{{pct}}\')"""',
        '__ratio_out := RunWaitOutput(__ratio_cmd)',
        'if (__ratio_out != "") {',
        '    StringSplit, __ratio_parts, __ratio_out, `,',
        f'    {store_count} := __ratio_parts1',
        f'    {store_ratio} := __ratio_parts2',
        '}',
    ]
    if op != "none":
        op_map = {"lte": "<=", "gte": ">=", "lt": "<", "gt": ">", "eq": "="}
        ahk_op = op_map.get(op, "<=")
        lines.extend([
            f"if ({store_ratio} {ahk_op} {val}) {{",
            f'    Log("gauge condition met: " . {store_ratio} . " {ahk_op} {val}")',
            "}",
        ])
    if sleep_after > 0:
        lines.append(f"Sleep, {sleep_after}")
...
def render_step(
    ...
    if action == "color_ratio":
        return render_color_ratio(step)
    return [f"; unknown action {action!r}"]


def render_dry_run_preview(step: Dict[str, Any], step_index: int, total_steps: int) -> List[str]:
    action = str(step.get("action") or "")
    next_step = int(step.get("on_success") or (step_index + 1 if step_index < total_steps else 0))
    repeat = int(step.get("repeat") or 1)
    repeat_var = normalize_variable_name(step.get("repeat_var"))
    detail_expression = (
        f'"action={ahk_quote(action)}; next={next_step}; repeat=" . {repeat_var}'
        if repeat_var
        else f'"action={ahk_quote(action)}; next={next_step}; repeat={repeat}"'
    )
    lines = [
        f'Log("dry-run action suppressed: {ahk_quote(action)}")',
        f'TraceStep({step_index}, "드라이런", "DETAIL", {detail_expression})',
    ]
...
def render_macro_script(
    ...
    if start_step:
        ...
        lines.append(f"if (MacroRunResumeStep + 0 >= 1 and MacroRunResumeStep + 0 <= {total_steps})")
        lines.append("{")
        lines.append('    Log("checkpoint resume: step " . MacroRunResumeStep)')
        lines.append('    Goto, % "Step" . (MacroRunResumeStep + 0)')
        lines.append("}")
        lines.append("")
    # Map automatic fallback between distinct branch lanes:
    # If a step is in a branch lane (workflow_id or branch_chain) and has no explicit on_fail,
    # it automatically falls back to the first step of the next branch lane!
    branch_next_fallback: Dict[int, int] = {}
    distinct_lanes: List[List[int]] = []
    lane_keys: Dict[str, int] = {}
    for idx, s in enumerate(steps, start=1):
        wf = str(s.get("workflow_id") or "").strip()
        automation = s.get("_automation") if isinstance(s.get("_automation"), dict) else {}
        lane_key = wf if wf else (f"__chain_{automation.get('candidate_position')}" if automation.get("branch_chain") else "")
        if lane_key:
            if lane_key not in lane_keys:
                lane_keys[lane_key] = len(distinct_lanes)
                distinct_lanes.append([])
            distinct_lanes[lane_keys[lane_key]].append(idx)
    ...
    for count, step in enumerate(steps, start=1):
        ...
        on_fail = step.get("on_fail")
        if not on_fail and count in branch_next_fallback:
            on_fail = branch_next_fallback[count]
        ...
        node_retry_count = max(0, min(100, int(step.get("node_retry_count") or 0)))
        node_retry_delay = max(10, min(600_000, int(step.get("node_retry_delay") or 250)))
        if action == "flow_control":
            repeat = 1
            repeat_var = ""
            on_success = None
        ...
        lines.append(f'StuckGuard_Check({count}, "{ahk_quote(str(label))}")')
        lines.append(f'Log("step start: {count} | {ahk_quote(str(label))}")')
        lines.append(f'TraceStep({count}, "{ahk_quote(str(label))}", "START")')
        lines.append(f'if (__node_retry_{count} = "")')
        lines.append(f"    __node_retry_{count} := 0")
        lines.extend(render_subflow_assignments(step.get("_subflow_inputs")))
        if repeat_var:
        ...
        suppressed_in_dry_run = action in {
            "mouse_click",
            "inactive_click",
            "type_text",
            "browser_action",
        ...
        if suppressed_in_dry_run:
            lines.append("if (MacroDryRun)")
            lines.append("{")
            lines.extend("    " + line for line in render_dry_run_preview(step, count, total_steps))
            lines.append("}")
            lines.append("else")
            lines.append("{")
            lines.extend("    " + line for line in render_step(step, assets, count, browser_fast))
            lines.append("}")
        else:
        ...
        if action in {"image_search", "screen_condition", "ocr", "datetime_condition"}:
            ...
            if node_retry_count:
                ...
                lines.append("    {")
                lines.append(f"        __node_retry_{count} += 1")
                lines.append(
                    f'        TraceStep({count}, "{ahk_quote(str(label))}", "RETRY", "attempt=" . __node_retry_{count} . "/{node_retry_count}")'
                )
                lines.append('        SetRunResult("RUNNING", "RETRYING", "노드 재시도 중")')
                lines.append(f"        Sleep, {node_retry_delay}")
                lines.append(f"        Goto, Step{count}")
                lines.append("    }")
            lines.append(f"    __node_retry_{count} := 0")
            lines.extend("    " + line for line in render_subflow_failure(step))
            ...
            lines.append(f"__node_retry_{count} := 0")
            lines.extend(render_subflow_success(step))
            lines.append(f"MarkStepSuccess({count}, {on_success})")
            lines.append(f'TraceStep({count}, "{ahk_quote(str(label))}", "SUCCESS")')
            lines.extend(render_edge_conditions(step, count, "success"))
            if on_success_delay > 0:
                lines.append(f"Sleep, {on_success_delay}")
            ...
            lines.append(f"__node_retry_{count} := 0")
            lines.extend(render_subflow_success(step))
            lines.append(f"MarkStepSuccess({count}, {count + 1 if count < total_steps else 0})")
            lines.append(f'TraceStep({count}, "{ahk_quote(str(label))}", "SUCCESS")')
            if action in {"set_var", "calc_var"}:
