...
'''
    ...
    def run_macro(self, name: str, turbo: bool = False) -> subprocess.Popen[Any]:
        ...
        return self._launch_macro_payload(name, payload, script, checkpoint_name=name, resume=True, turbo=turbo)

    def run_macro_from_step(self, name: str, step_index: int, turbo: bool = False) -> subprocess.Popen[Any]:
    ...
    def run_macro_step(self, name: str, step_index: int, turbo: bool = False) -> subprocess.Popen[Any]:
        payload = deepcopy(self.load_macro(name))
        steps = payload.get("steps") or []
        if not 1 <= int(step_index) <= len(steps):
        ...
        engine.export_macro_payload(payload, script)
        return self._launch_macro_payload(f"{name} · {step_index}번 단계", payload, script, resume=False, turbo=turbo)
