...
class UiSmokeTests(unittest.TestCase):
    ...
    def test_action_editor_multi_asset_picker_round_trips_and_forces_opencv(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertFalse(editor.widgets["image_search"]["click.click_image"].isChecked())
            step = editor.build_step()
            self.assertEqual(["첫 이미지", "둘째 이미지"], step["assets"])
            self.assertEqual({"첫 이미지": [21, -7], "둘째 이미지": [-24, 35]}, step["asset_offsets"])
            self.assertTrue(step["click"]["click_offset"])
    ...
    def test_workflow_steps_render_as_separate_lanes(self) -> None:
        from PySide6 import QtWidgets
        from macro_studio.node_editor import NodeCanvas
...
class SmartRecordingUiTests(unittest.TestCase):
    @staticmethod
    def _sample_bmp() -> str:
        from PySide6 import QtCore, QtGui, QtWidgets
    ...
    def test_builder_keeps_smart_recording_without_ai_recording_tools(self) -> None:
        os.environ.setdefault("QT_QPA_PLATFORM", "offscreen")
        from PySide6 import QtWidgets
        from macro_studio.builder import BuilderPage
        from macro_studio.repository import MacroRepository
        ...
        with tempfile.TemporaryDirectory() as directory:
            builder = BuilderPage(MacroRepository(Path(directory)))
            labels = [button.text() for button in builder.findChildren(QtWidgets.QAbstractButton)]
            self.assertIn("● 스마트 녹화", labels)
            self.assertIn("▤ 최근 녹화 검토", labels)
            self.assertIn("⑂ 선택 노드 분기 묶기", labels)
            joined = "\n".join(labels)
            self.assertNotIn("AI 매크로 녹화", joined)
            self.assertNotIn("AI 분석 녹화", joined)
    ...
    def test_repository_removes_only_retired_ai_recording_artifacts(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            obsolete = [
                root / ".automation" / "ai-packages" / "old" / "recording.mp4",
                root / ".automation" / "last-ai-recording.json",
                root / "exports" / "ai-recordings" / "old.zip",
                root / ".backups" / "studio-2.44.0-before-embedded-ai-prompt-20260901" / "old.bin",
                root / "macro_studio" / "ai_recording.py",
            ]
            preserved = [
                root / ".automation" / "last-recording.json",
                root / "exports" / "failure_captures" / "failure.png",
                root / ".backups" / "studio-2.38.0-before-datetime-ui-20260831" / "keep.bin",
            ]
            for path in [*obsolete, *preserved]:
                path.parent.mkdir(parents=True, exist_ok=True)
                path.write_bytes(b"test")
            ai_macro = root / "macros" / "AI 초안.json"
            ai_history = root / ".history" / "AI 초안" / "version.json"
            ai_archive = root / ".archive" / "macros" / "AI 초안-archived.json"
            normal_macro = root / "macros" / "일반 매크로.json"
            for path in (ai_macro, ai_history, ai_archive):
                path.parent.mkdir(parents=True, exist_ok=True)
                path.write_text(
                    json.dumps({"name": "AI 초안", "meta": {"ai_draft": True}, "steps": []}, ensure_ascii=False),
                    encoding="utf-8",
                )
            normal_macro.parent.mkdir(parents=True, exist_ok=True)
            normal_macro.write_text(
                json.dumps({"name": "일반 매크로", "meta": {}, "steps": []}, ensure_ascii=False), encoding="utf-8"
            )
            MacroRepository(root)
            self.assertTrue(all(not path.exists() for path in obsolete))
            self.assertTrue(all(path.is_file() for path in preserved))
            self.assertTrue((root / ".automation" / "cleanup-2.46.0.done").is_file())
            self.assertFalse(ai_macro.exists())
            self.assertFalse(ai_history.parent.exists())
            self.assertFalse(ai_archive.exists())
            self.assertTrue(normal_macro.is_file())
