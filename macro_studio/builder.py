...
class BuilderPage(QtWidgets.QWidget):
    ...
    def _build_steps_panel(self) -> QtWidgets.QWidget:
        ...
        duplicate_node_btn = QtWidgets.QPushButton("노드 복제")
        duplicate_node_btn.clicked.connect(lambda: self._duplicate_node_from_graph(self.node_canvas.selected_index()))
        sequential_btn = QtWidgets.QPushButton("순차 연결")
        sequential_btn.clicked.connect(self._connect_sequentially)
        start_btn = QtWidgets.QPushButton("시작 지정")
        start_btn.clicked.connect(lambda: self._set_graph_marker("start"))
        end_btn = QtWidgets.QPushButton("종료 지정")
        end_btn.clicked.connect(lambda: self._set_graph_marker("end"))
        delete_node_btn = danger_button("노드 보관")
        delete_node_btn.clicked.connect(lambda: self._delete_node_from_graph(self.node_canvas.selected_index()))
        test_node_btn = QtWidgets.QPushButton("▶ 선택 단계 테스트")
        test_node_btn.clicked.connect(self._test_selected_step)
        save_block_btn = QtWidgets.QPushButton("블록 저장")
        save_block_btn.setToolTip("선택한 노드 묶음을 다른 매크로에서 재사용")
        save_block_btn.clicked.connect(self._save_selected_block)
        add_block_btn = QtWidgets.QPushButton("＋ 블록 추가")
        add_block_btn.clicked.connect(self._insert_automation_block)
        ...
        node_actions.addWidget(end_btn)
        node_actions.addWidget(test_node_btn)
        node_actions.addWidget(save_block_btn)
