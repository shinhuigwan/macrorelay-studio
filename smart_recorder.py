...
class Recorder:
    def __init__(
        ...
        self.region_capture_vk = int(region_capture_vk)
    ...
    def _mouse_proc(self, code: int, message: int, data_ptr: int) -> int:
        ...
        if code == HC_ACTION and message in down_messages | up_messages:
            ...
            is_f4_click = self._f4_key_down or bool(user32.GetAsyncKeyState(self.region_capture_vk) & 0x8000)
            self.emit(
                {
                    "type": "screen_verification" if self.right_click_condition and message == WM_RBUTTONDOWN else "mouse",
                    "source_control": "F4_Click" if is_f4_click else ("RightClick" if self.right_click_condition and message == WM_RBUTTONDOWN else "Mouse"),
                    "with_search_region": is_f4_click,
                    "event_id": event_id,
                    "button": button,
    ...
    def _keyboard_proc(self, code: int, message: int, data_ptr: int) -> int:
        if code == HC_ACTION and message in {WM_KEYDOWN, WM_KEYUP, WM_SYSKEYDOWN, WM_SYSKEYUP}:
            ...
            if message not in {WM_KEYDOWN, WM_SYSKEYDOWN}:
                if vk == self.region_capture_vk:
                    self._f4_key_down = False
                return int(user32.CallNextHookEx(self.keyboard_hook, code, message, data_ptr))
            if vk == self.stop_vk:
                user32.PostQuitMessage(0)
                return 1
            if vk in {self.capture_vk, self.region_capture_vk}:
                if vk == self.region_capture_vk:
                    self._f4_key_down = pressed
                if self.gate_down and pressed:
                    ...
                    self.emit_control(
                        {
                            ...
                            "vk": vk,
                            "with_search_region": (vk == self.region_capture_vk),
                        }
                    )
                return 1
...
def main() -> int:
    ...
    parser.add_argument("--region-capture-vk", type=int, default=VK_F4)
