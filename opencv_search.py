...
def match_frame(
    ...
    *,
    ...
    for prepared in templates:
        ...
        score_val = float(max_val)
        if template_mask is not None and score_val > 0.2:
            patch = gray_img[max_loc[1]:max_loc[1] + tpl_h, max_loc[0]:max_loc[0] + tpl_w]
            if patch.shape == gray_tpl.shape:
                mask_bool = template_mask > 8
                t_vals = gray_tpl[mask_bool].astype(np.float32)
                p_vals = patch[mask_bool].astype(np.float32)
                std_t = float(np.std(t_vals))
                std_p = float(np.std(p_vals))
                if std_t > 1e-2 and std_p > 1e-2:
                    cov = float(np.mean((t_vals - np.mean(t_vals)) * (p_vals - np.mean(p_vals))))
                    pearson = cov / (std_t * std_p)
                    score_val = min(score_val, max(0.0, pearson))
                elif std_t > 1e-2:
                    score_val = 0.0
        full_location = (max_loc[0] - origin_x, max_loc[1] - origin_y)
        candidate = (score_val, full_location, full_width, full_height)
