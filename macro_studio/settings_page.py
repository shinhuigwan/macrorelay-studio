...
class SettingsPage(QtWidgets.QWidget):
    ...
    def _send_test_remote_notify(self) -> None:
        try:
            config = self.remote.load()
            if not config.get("enabled"):
                QtWidgets.QMessageBox.warning(
                    self,
                    "원격 알림 테스트",
                    "원격 제어 기능이 꺼져 있습니다.\n먼저 '원격 제어' 체크박스를 켜고 설정을 저장해주세요.",
                )
                return
            from remote_common import post_agent_event
            res = post_agent_event(
                config,
                "notification",
                "Studio와 스마트폰 연동이 정상 작동 중입니다! 🔔",
                {"title": "🔔 원격 알림 테스트", "level": "info"},
            )
            if res.get("ok"):
                QtWidgets.QMessageBox.information(
                    self,
                    "알림 전송 성공",
                    "스마트폰으로 테스트 알림이 전송되었습니다!\n\n스마트폰 화면에서 팝업 알림과 소리/진동을 확인하세요.",
                )
                self.status.emit("휴대폰으로 테스트 알림을 전송했습니다.")
            else:
                QtWidgets.QMessageBox.warning(
                    self,
                    "알림 전송 실패",
                    f"알림 전송 실패: {res.get('error', '알 수 없는 오류')}\n중계 서버 및 에이전트 연결 상태를 확인해주세요.",
                )
        except Exception as e:
            QtWidgets.QMessageBox.critical(self, "알림 전송 오류", f"오류가 발생했습니다: {e}")
