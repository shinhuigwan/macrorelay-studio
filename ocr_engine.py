...
def handle_ocr(state: EngineState, req: dict) -> dict:
    ...
    try:
        ...
        # 2) OCR 엔진 실행 (지연 전처리 & 적응형 오토스케일링으로 속도/정확도 대폭 향상)
        candidates = []
        ...
        if paddle_requested:
            ...
            if paddle_engine and paddle_engine.is_loaded and language_supported:
                try:
                    # 작은 UI 폰트 정확도 향상: 높이가 45px 미만이면 2배, 70px 미만이면 1.5배 바이큐빅 보간 확대
                    h, w = image.shape[:2]
                    scale = 2.0 if h < 45 else (1.5 if h < 70 else 1.0)
                    if scale > 1.0:
                        try:
                            import cv2
                            scaled_img = cv2.resize(image, (int(w * scale), int(h * scale)), interpolation=cv2.INTER_CUBIC)
                        except Exception:
                            scaled_img = image
                            scale = 1.0
                    else:
                    ...
                    paddle_result = paddle_engine.recognize(scaled_img, scale=scale, lang=lang)
                    if _postprocess_mod:
                        candidates.append(
        ...
        # 3) Tesseract (보조 엔진): PaddleOCR 미만족 또는 Tesseract 지정 시에만 지연 전처리 실행
