...
def set_autostart_enabled(enabled: bool) -> bool:
    ...
    try:
        ...
        if enabled:
            cmd = r'powershell.exe -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File "C:\Users\shin\app\macro_tool\run_quickslot.ps1"'
...
class RadialPieMenuWidget(QtWidgets.QWidget):
    ...
    def _init_default_items(self) -> None:
        default_keys = ["prev_page", "next_page", "settings", "emergency", "refresh", "studio", "topmost", "minimize"]
        self.items = []
...
class RadialWheelPreviewWidget(QtWidgets.QWidget):
    ...
    def __init__(self, parent: Optional[QtWidgets.QWidget] = None):
        ...
        self.setMouseTracking(True)
        self.items_keys: List[str] = ["prev_page", "next_page", "settings", "emergency", "refresh", "studio", "topmost", "minimize"]
...
class QuickSlotDeckSettingsDialog(QtWidgets.QDialog):
    ...
    def _init_grid_tab(self) -> None:
        ...
        self.compact_fit_check = QtWidgets.QCheckBox("등록된 슬롯 수에 맞춰 프로그램 창 크기 자동 맞춤 (Auto-Fit Compact Window)")
        self.compact_fit_check.setStyleSheet("font-weight: 700; color: #38BDF8;")
        self.compact_fit_check.setChecked(True)
        self.compact_fit_check.toggled.connect(self._on_compact_fit_toggled)
    ...
    def _load_settings(self) -> None:
        ...
        try:
            ...
            self.compact_fit_check.setChecked(bool(self.main_window.config.get("compact_auto_fit", True)))
            ...
            radial_keys = list(self.main_window.config.get("radial_items") or ["prev_page", "next_page", "settings", "emergency", "refresh", "studio", "topmost", "minimize"])
    ...
    def _apply_settings(self) -> None:
        ...
        self.main_window.config["compact_auto_fit"] = self.compact_fit_check.isChecked()
...
class RadialMenuIconButton(QtWidgets.QPushButton):
    ...
    def __init__(self, parent: Optional[QtWidgets.QWidget] = None):
        ...
        icon_paths = [
            r"C:\Users\shin\Desktop\아이콘\레디얼.png",
            r"C:\Users\shin\app\macro_tool\radial-icon-transparent.png",
            r"C:\Users\shin\app\macro_tool\radial-icon.png",
        ]
        pix = QtGui.QPixmap()
        for p in icon_paths:
            if os.path.exists(p):
...
class QuickSlotDeckWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, repository: Optional[MacroRepository] = None, parent: Optional[QtWidgets.QWidget] = None):
        ...
        icon_path = Path(r"C:\Users\shin\app\macro_tool\quickslot-deck.ico")
        if icon_path.exists():
    ...
    def _init_ui(self) -> None:
        ...
        if not self.geometry().isValid() or self.width() < 500:
    ...
    def _setup_watcher(self) -> None:
        ...
        if os.path.exists(hotkeys_file):
            ...
            self.watcher.fileChanged.connect(lambda _path: self.refresh_slots())
    ...
    def refresh_slots(self) -> None:
        ...
        for i in range(pageSize):
            ...
            if slot_idx < len(slots):
                ...
                btn.set_slot_data("", "", "hybrid", False)
        ...
        if bool(self.config.get("compact_auto_fit", True)):
            active_count = sum(
                1 for btn in self.buttons
                if btn.macro_name or not load_pixmap_from_config(btn.custom_icon_config).isNull() or bool(btn.custom_icon_config.get("emoji"))
            )
            if active_count > 0:
    ...
    def _is_macro_running(self, macro_name: str) -> bool:
        if not macro_name:
            return False
        proc = self.active_processes.get(macro_name)
    ...
    def _execute_slot_macro(self, slot_index: int, macro_name: str) -> None:
        ...
        try:
            ...
            self.active_processes[macro_name] = proc
    ...
    def _stop_slot_macro(self, slot_index: int, macro_name: str) -> None:
        proc = self.active_processes.pop(macro_name, None)
        if proc:
            try:
                proc.terminate()
            except Exception:
    ...
    def refresh_states(self) -> None:
        ...
        finished = [name for name, proc in self.active_processes.items() if proc.poll() is not None]
        for name in finished:
            del self.active_processes[name]

        running_count = len(self.active_processes)
        if running_count > 0:
            names = ", ".join(self.active_processes.keys())
            if hasattr(self, "status_title") and self.status_title:
    ...
    def _open_studio(self) -> None:
        script = self.repository.root / "run_studio.py"
        if script.exists():
            subprocess.Popen([sys.executable, str(script)], cwd=str(self.repository.root))
